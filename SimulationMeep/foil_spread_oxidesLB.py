...
# === Directories ===
output_dir = "SimFiles/Pipe_2D_ImperfectFoil_MultiOxide_Backscatter"
...
oxide_radius = 0.1
num_oxides = 5
vertical_spacing = 0.4  # vertical spacing between oxide points
...
oxide_size_display = 150  # controls visual diameter, not physical size
ax.scatter(
...
ax.scatter(
    recv_center.x,
    recv_center.y,
    s=oxide_size_display,   # match oxide sources
    c='red',
    marker='o',
...
ax.set_xlabel('x (a.u.)')
ax.set_ylabel('y (a.u.)')
ax.set_title('2D: Multiple Oxide Sources Backscatter in Pipe')
ax.legend(loc='upper right', frameon=False)
ax.set_aspect('equal', adjustable='box')

plt.tight_layout()
plt.savefig(os.path.join(output_dir, "backscatter_field_multi_oxide.png"), dpi=300)
plt.show()
